...
<script>
    ...
    function validarNombre() {
        ...
        var nombreValido = /^[a-zA-ZñÑáéíóúÁÉÍÓÚ\s0-9]+$/.test(nombre);
    ...
    function validarAutor() {
        ...
        var autorValido = /^[a-zA-ZñÑáéíóúÁÉÍÓÚ\s0-9]+$/.test(autor); // Solo letras y espacios
...
function enviarFormularioConProgreso() {
    var form = document.getElementById('registroNoticiaForm');
    var formData = new FormData(form);

    // Crear un nuevo XMLHttpRequest
    var xhr = new XMLHttpRequest();
    
    // Configurar el progreso de la carga
    ...
    // Configurar qué hacer cuando el formulario se haya enviado
    xhr.addEventListener("load", function(event) {
        mostrarSweetAlert('success', 'Éxito', 'La noticia ha sido registrada exitosamente.');
        setTimeout(function() {
            window.location.reload(); // Recargar la página o redirigir si es necesario
        }, 1500);
    });

    // Configurar qué hacer en caso de error
    xhr.addEventListener("error", function(event) {
        mostrarSweetAlert('error', 'Error', 'Hubo un problema al enviar el formulario.');
    });

    // Abrir la conexión y enviar el formulario
    xhr.open("POST", form.action, true);
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest'); // En caso de que sea necesario para tu back-end
    xhr.send(formData);
}
